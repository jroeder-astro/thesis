...
therefore staying spherically symmetric. In terms of the stress-energy tensor we are left with ($c=1$):
\begin{equation}
	T_{\mu\nu}=\left(\epsilon + P\right)u_\nu u_\mu-Pg_{\mu\nu}
\end{equation}
Spherical symmetry leads to a certain form of the metric, which then gives us the stress-energy tensor omponents:
\begin{equation}
	T_{\mu\nu} = diag(\epsilon e^{\nu(r)}  , \ P e^{\lambda(r)},\ Pr^2,\ Pr^2sin^2(\theta)    )
\end{equation}
In conclusion we get the full TOV equation:

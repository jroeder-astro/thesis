...
\begin{equation*}\label{eq:gen_eos}
	f(p,V,T)=0
\end{equation*}
with, in tis case, pressure $p$, Volume $V$ and Temperature $T$. The nature of $f$ (the actual EOS) is then derived from a model (e.g. nuclear matter) to properly describe the state of matter at hand, be it a fluid, solid or of an exotic type. Here, starting on the outside of a neutron star, states are still in or not far from what can be experimentally observed. Towards the inner regions, exotic states and their impact on the EOS are still subject to investigation.\\

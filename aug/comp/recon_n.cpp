...
//	      if( fabs(y_tau[0]-y[i1][0]) <= pow(10., -4.))	
    ...
    else
    {
      ...
      continue;  
           ...
           else 
	   {
              ...
              /*log*/      << fabs(t[i1-1] - MR_rel[mcount][0]) << endl; 
              /*log*/// cout << " alpha[3]_b = " << alpha[3] << endl;              
              /*log*/ cout << "   Slope =  line  "<<(alpha[3]-alpha[1])/(alpha[2]-alpha[0]) << endl;
    ...
    if(!flag_s)
    {
      ...
      printf("diff radius %f %f %f %f\n",slope_step,diff0_s,t[i1-1],MR_rel[mcount][0]);
      flag_s=true;
      continue;
    }
    
    diff_s = t[i1-1] - MR_rel[mcount][0];
    printf("diff radius %f %f %f %f\n",slope_step,diff_s,t[i1-1],MR_rel[mcount][0]);
    
    if(fabs(diff_s)<.005) {break;}
    if(diff0_s*diff_s>0) {continue;}
    
    slope_step/=10;
    
    if(slope_step<1e-5) {break;}

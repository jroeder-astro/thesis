  ...
  for(i1 = 0; i1 < num_steps_max; i1++)
    {
      ...
      // y(t)   --> \tau   y_{\tau}(t+\tau)
      RK_step(y[i1], r[i1], y_tau, tau);

      // y(t)   --> \tau/2   --> \tau_2   y_{2 * \tau / 2}(t+\tau)
      RK_step(y[i1], r[i1], y_tmp, 0.5*tau);
      RK_step(y_tmp, r[i1]+0.5*tau, y_2_x_tau_over_2, 0.5*tau);
      ...
      for(i2 = 1; i2 < N; i2++)
	{
      ...
      d1 = 0.9 * pow(d_abs_max / delta_abs, 1.0 / (((double)order)+1.0));

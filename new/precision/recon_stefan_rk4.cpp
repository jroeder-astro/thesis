...
//      /* && Error check?? Same problem as with my code then */    ) {
    ...
    R_rel_err = fabs(t[i1-1]-MR_rel[mcount-1][0])/MR_rel[mcount-1][0];
    M_rel_err = fabs(y[i1-1][1]/1.4766 - MR_rel[mcount-1][1])
                /MR_rel[mcount-1][1];
    ...
    cout << "out: " << t[i1-1] << "," << y[i1-1][1]/1.4766 
         ...
         << "," << slope << "," << R_rel_err << ","<< M_rel_err << endl;
...
void tov_euler(double *y_t, double t, 
  ...
  for (i1 = 0; i1 < N; i1++) {
    ...
    y_t_plus_tau[i1] = y_t[i1] + 1./6. * (k1[i1]+2.*(k2[i1]+k3[i1])+k4[i1]);
  }
}

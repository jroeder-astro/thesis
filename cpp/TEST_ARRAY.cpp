...
//  while (M <= 1.7) {
      ...
      if (fscanf(TOV, "%lf,%lf", &M, &R) == EOF) exit(0); 
...
cout << "hi 257\n";
	   ...
	   vector<double>::iterator itP1;
	    itP1 = Presult.begin()+n+1;
	    itP1 = Presult.insert(itP1, REOS[n][0]); 
	 
	   vector<double>::iterator itE1;
	    itE1 = Eresult.begin()+n+1;
	    itE1 = Eresult.insert(itE1, REOS[n][1]); 
